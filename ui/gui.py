...
#         """✅ Fetch username dynamically from the backend before making payment requests."""
#         try:
#             response = requests.get(f"{self.api_url}/get_username")  # ✅ Make sure this endpoint exists in your backend
            
#             if response.status_code == 200:
#                 data = response.json()
#                 self.username = data.get("username")
#                 print(f"✅ Username fetched: {self.username}")
#             else:
#                 print("⚠ Failed to fetch username from backend.")

#         except requests.exceptions.RequestException as e:
#             print(f"❌ Error fetching username: {e}")

#     # check payment for pi
...
#     #                 self.transaction_id = None  # ✅ Clear transaction_id after successful verification
...
#     def check_payment_status(self):
#         """✅ Fetch the latest username before checking payment status"""
#         self.fetch_username()  # Ensure username is updated before making request

#         if not self.username:
#             print("❌ Username is not available. Cannot check payment status.")
#             return
...
#         self.success_label.config(text=f"✅ Payment Successful!\nEnjoy your {product}!", fg="green")
#         self.success_label.pack(pady=20)
...
#             print(f"❌ Error fetching username: {e}")
    ...
    #             data = response.json()
    #             if data.get("status") == "success" and "product" in data:
    #                 self.display_success_message(data["product"])  # ✅ Show latest success
    #             else:
    #                 self.success_label.config(text="⌛ Waiting for Payment...", fg="orange")
    #         else:
    #             self.success_label.config(text="⚠ No new transaction found.", fg="red")
    #     except Exception as e:
    #         self.success_label.config(text="❌ Payment Failed!", fg="red")
    ...
    # def display_success_message(self, product):
    #     """Display the payment success message on a new screen."""
    #     self.clear_screen()

    #     # Create a new frame for the success message screen
    #     success_frame = tk.Frame(self.root)
    #     success_frame.pack(expand=True, fill="both")

    #     self.success_label = tk.Label(success_frame, text=f"✅ Payment Successful!\nEnjoy your {product}!", fg="green",
    #                               font=("Arial", 16, "bold"))
    #     self.success_label.pack(pady=40)

    #     # After 3 seconds, show the dispensing message
    #     self.root.after(3000, self.display_dispensing_message)
    ...
    def display_success_message(self, product):
        """Show a success message and return to the main screen after 10 seconds."""
        self.success_label.config(text=f"✅ Payment Successful!\nEnjoy your {product}!", fg="green")
        self.success_label.pack(pady=20)

        # Hide success message and return to main screen after 10 seconds
        self.root.after(10000, lambda: self.hide_success_message())

    def hide_success_message(self):
        """Clear the success message and return to the main screen."""
        self.success_label.pack_forget()

    def update_payment_status(self):
        """Continuously update the payment status every 5 seconds."""
        self.check_payment_status()
        self.root.after(20000, self.update_payment_status)